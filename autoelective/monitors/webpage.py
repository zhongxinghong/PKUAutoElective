...
cout = ConsoleLogger("webpage")
ferr = FileLogger("webpage.error")
...
def _root():
    ...
    for r in sorted(current_app.url_map.iter_rules(), key=lambda r: r.rule):
        line = "{method}  {rule}".format(method=','.join(
...
def run():
    monitor.run(
        host=config.monitor_webpage_host,
        port=config.monitor_webpage_port,
        debug=True,
        use_reloader=False,
    )
